...
function Get-InstallationFiles {
    ...
    process {
        ...
        if ($Files.Length -eq 1) {
            ...
            $downloadFileDefinition = "function downloadfile { ${function:downloadfile} }"
            foreach ($file in $files) {
                # TODO: Test what happens if one call fails
                $jobs += Start-ThreadJob -Name $file.DownloadPath -ScriptBlock {
                    . ([ScriptBlock]::Create($using:downloadFileDefinition))
                    downloadfile -Feature $using:file
                }
            }
